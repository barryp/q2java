 ...
 * <p>
 * The collection is defined in terms of a root element and the elements to
 * look for under this root. Only elements of the specified type are contained
 * in the collection. Elements are returned by index or by identifier (the
 * <TT>id</TT> attribute). The collection is live, meaning that changes to the
 * document tree are immediately reflected in the collection. The collection
 * is not optimized for traversing large document trees.
 * <p>
		...
		// return this element; otherwise, traverse it's children.
                if ( node instanceof Element )
                {
                    if ( collectionMatch( (Element) node, name ) )
                        return node;
		    if ( ! collectionMatch( (Element) node, null ) && recurse() )
                    {
