...
/**
 * Guided missile that waits until it has a target before launching.
 *
 * @author Barry Pederson
 */
public class StandbyMissile extends GenericWidgetBody implements ServerFrameListener
	{
...
public void handleWidgetEvent(int event, Object extra) 
	...
	switch (event)
		...
		case WidgetBody.TARGET:
			...
			if ((!fRunning) && (fTarget != null))
				{
				Game.addServerFrameListener(this, 0, 0);
				fRunning = true;
				}

			if (fRunning && (fTarget == null) && (!fLaunched))
				{
				Game.removeServerFrameListener(this);
				fRunning = false;
				}
...
public void runFrame(int phase) 
	...
	if (fTarget != null)
		...
		// and the target
		angle =  newDir.angle(fDirection);

		// alter direction if necessary by interpolating between the current
...
//			double interp = ((fSpeed / 100) * (Math.PI/180.0))/angle;
	...
	if (!fLaunched)
		{
		// check if we've swivelled around to within 30 degrees of the target
		// and bail if not
		if (angle > (Math.PI / 6.0))
			return;

		// we're pointing somewhat towards the target, so actually launch
	...
	// bail if the rocket has been around too long
	if (++fCount > TIMEOUT)
		fireWidgetEvent(WidgetBody.TERMINATED);
	}
/**
